...
- **No hard-coded brand colors.** File outputs get their palette from `shared/design`, starting from the agent profile's colors (buyer blue / seller orange by default). Every color is a shade or tint of that brand color, or black, gray or white; a different hue only where the color carries meaning: status (good / caution / risk, fixed) and party coding (Buyer / Seller / Both). The subject home is black, never a second hue.
- **Print-light PDFs:** no filled section bars, table headers, zebra rows, hero panels or callouts. Use colored type, rules, thin accent bars and outlined boxes with no fill; fill only where the fill is the data (chart marks, timeline bars, meters, small pills). One exception: a chart's takeaway box (the one line a skimming reader must see) may use the faint `--brand-callout` tint, at most one per chart. See [docs/architecture.md](docs/architecture.md#brand-colors).
