...
**From the agent**
10. The MLS CMA export (CSV) of single-family homes nearby: sales from about the last 6 months, plus active, pending, expired and canceled listings. For Stellar the columns are built in; another MLS needs its headers mapped (`--columns` and `export_columns`).
